...
module.exports = {
  ...
  createArtist: async (args) => {
    ...
    const newArtist = new Artist({
      ...
      artistAmountInfo: {
        ...
        totalInstallments:
          args.artistInput.artistAmountInfo.totalInstallments || 0,
      },
    });
  ...
  updateArtist: async (args) => {
    ...
    if (genres != undefined) {
      ...
      genres.forEach((genre) => {
        if (!originalList.includes(genre.toLowerCase()))
          filteredList.push(genre);
      });
    ...
    if (topics != undefined) {
      ...
      topics.forEach((topic) => {
        if (!originalList.includes(topic.toLowerCase()))
          filteredList.push(topic);
      });
    ...
    if (args.updateArtistInput.artistAmountInfo !== undefined) {
      ...
      if (args.updateArtistInput.artistAmountInfo.taxInfo !== undefined) {
        const originalTaxInfo = artistToBeUpdated.artistAmountInfo.taxInfo.map(
          (item) => {
      ...
      artistToBeUpdated.artistAmountInfo = {
        ...
        totalAmount:
          baseAmount || artistToBeUpdated.artistAmountInfo.totalAmount,
        totalInstallments:
          totalInstallments ||
          artistToBeUpdated.artistAmountInfo.totalInstallments,
        isPaid: isPaid || artistToBeUpdated.artistAmountInfo.isPaid,
        pendingAmount:
          baseAmount || artistToBeUpdated.artistAmountInfo.pendingAmount,
  ...
  updateArtistTaxInfo: async (args) => {
    ...
    const originalTaxInfo = taxInfoToBeUpdated.artistAmountInfo.taxInfo.id(
      args.taxInfoId
    );
    originalTaxInfo.taxName =
      args.updateTaxInfo.taxName || originalTaxInfo.taxName;
