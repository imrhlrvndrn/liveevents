...
const transformUser = async (user) => {
    return {
        ...
        _id: user.id,
        password: null,
        createdEvents: getAllCreatedEvents.bind(this, user._doc.createdEvents),
...
const transformOrganization = async (organization) => {
    return {
        ...organization._doc,
        _id: organization.id,
        owner: getUserById.bind(this, organization._doc.owner),
        createdEvents: getAllCreatedEvents.bind(this, organization._doc.createdEvents),
        members: getMembersOfOrganization.bind(this, organization._doc.members),
    };
};
...
const getMembersOfOrganization = async (memberIds) => {
    const returnedMembers = await User.find({ _id: { $in: memberIds } });

    return returnedMembers.map((member) => {
        return transformOrganization(member);
    });
};

const getAttendeesOfEvent = async (attendeeIds) => {
    const returnedBookedByUsers = await Booking.find({
        _id: { $in: attendeeIds },
    });
...
const getAllBookedEventsOfUser = async (bookedEventIds) => {
    const returnedBookedEvents = await Booking.find({
...
const getAllCreatedEvents = async (createdEventIds) => {
    const returnedCreatedEvents = await Event.find({
...
module.exports = {
    ...
    transformOrganization: transformOrganization,
    ...
    getMembersOfOrganization: getMembersOfOrganization,
    getAttendeesOfEvent: getAttendeesOfEvent,
    getAllBookedEventsOfUser: getAllBookedEventsOfUser,
    getAllCreatedEvents: getAllCreatedEvents,
